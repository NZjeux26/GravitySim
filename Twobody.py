...
### THINGS TO ADD ####
### barycentres mark
### For loop so each object is checked against the other without having to be manually added
### Objects initional velocity is automatically calculated for circular orbit at that height
### Plot lines for all objects DONE
...
satellite = PointMass(
    position=[320e3,0.0,0.0], #the planet is the 0,0,0 (centre) so the position is now realtive to that. Currently that 320km + Planet Radius
    mass=100,
    radius=1,
...
satellite.velocity[1] = - 45600#-get_starting_velocity(satellite)
...
while running:
    ...
    #Euler Intergration. #need to make this more like a for loop for each combo of obejct (n_body) or just between object-planet planet-object
    #euler_integration(satellite, planet, dt)
    #euler_integration(moon,planet,dt)
    #euler_integration(planet, moon,dt)
    
    #euler_integration(satellite,moon,dt)
    ...
    #Add old position to array for drawing ### Need to make this to go though each object (if onscreen) in a 0=sat 1=moon 3=planet offset 
    pos_count +=1
    if pos_count % 10 == 0:
        for obj in objects:
